...
class VisionTransformer(nn.Module):
    """
    Vision Transformer with FFN adapters (One-A backbone).
    """
    ...
    def __init__(
        ...
        print("Using ViT backbone with adapters (One-A).")
        ...
        # Classifier heads (we do not use them in One-A, but keep for completeness)
        self.head = (
        ...
        # Prompt tuning (not used in One-A, but config-compatible)
...
# -------------------------------------------------------------------------
# Factory functions for One-A ViT backbones
# -------------------------------------------------------------------------

def _load_and_remap_vit_state_dict(model_name: str):
    """
    Load a timm ViT model and remap qkv / MLP weights to match One-A ViT.
    """
...
def vit_base_patch16_224_onea(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """
    ViT-B/16 backbone on ImageNet-1k, adapted for One-A (adapters in FFN).
    """
...
def vit_base_patch16_224_in21k_onea(
    pretrained: bool = False, **kwargs
) -> VisionTransformer:
    """
    ViT-B/16 backbone pre-trained on ImageNet-21k, adapted for One-A.
    """
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
