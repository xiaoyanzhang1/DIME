...
class BaseLearner(object):
    ...
    def _evaluate(self, y_pred, y_true):
        ret = {}
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes, self.args["cls_per_task"][0], self.args["cls_per_task"])
        ret["grouped"] = grouped
    ...
    def eval_task(self):
        ...
        self.logger.info(f"[Task Eval] Inference completed in {elapsed:.2f} seconds.")
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            for k in range(1, m + 1):
                if len(vectors) == 0:
                    logging.warning(f"[Herding] Class {class_idx}: only selected {k-1}/{m} exemplars due to limited data.")
                    break
                S = np.sum(
                    exemplar_vectors, axis=0
                )  # [feature_dim] sum of selected exemplars vectors
